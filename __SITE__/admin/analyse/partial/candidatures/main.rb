  ...
  class Analyste
    class << self
      ...
      def validate user_id
        user.admin? || raise("Cette opération ne peut être effectuée que par un administrateur. Passez votre chemin.")
        analyste = User.get(user_id)
        set_bit_analyse( analyste, 3 )
        analyste.send_mail({
          ...
          <p>Et bien entendu, n'hésitez pas à vous mettre en relation avec les autres analystes
        ...
        __notice("La candidature de #{analyste.pseudo} (##{user_id}) a été acceptée.")
      end

      # Méthode pour refuser la candidature
      def refuser user_id
        motif = param(:motif_refus).nil_if_empty
        motif != nil || (return __error("Il faut impérativement argumenter votre refus."))
        analyste = User.get(user_id)
        set_bit_analyse(analyste, 2)
        analyste.send_mail({
        ...
        __notice("La candidature de #{analyste.pseudo} (##{user_id}) a été refusée. Le motif de son refus lui a été transmis.")
      end
      ...
      def li_candidat data
        ...
        <li class="candidat" id="candidat-#{data[:id]}">
          <div class="fright">
            ...
            <a onclick="document.querySelector('form#refus_form-#{data[:id]}').style.display='block';return false;" class="btn warning small">refuser</a>
          </div>
