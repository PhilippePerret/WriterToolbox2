...
class Site
  ...
  # Les trois listes de tâches produites par define_all_list_taches
  #
  attr_reader :list_taches_to_start, :list_taches_courantes, :list_last_taches_done
  ...
  def define_all_list_taches auteur, task_type
    ...
    @list_last_taches_done = build_liste_abs_taches(tdone,    habsworks, :done)
  end
  ...
  #
  TITRE_BY_TYPE = {
    ready:   'Tâches à démarrer',
    current: 'Tâches courantes',
    done:    'Tâches récemment achevées'
  }
  def build_liste_abs_taches htaches, habsworks, type_tache
    htaches.count > 0 || (return '')
    TITRE_BY_TYPE[type_tache].in_div(class: 'titre_listing_taches')+
