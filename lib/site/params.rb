...
class Site
  ...
  # variables d'instance (site.custom_params) que si on
  ...
  def params_session
    @params_session ||= session.instance_variable_get('@data').to_sym
  end
  # Paramètres de l'url
...
class ::String
  ...
  def as_hash_from_query_string
    ...
    self.split('&').each do |propvalue|
      ...
      h.merge! key.to_sym => ( site.real_value( URI.unescape( value.to_s ) ) )
    end
    return h
  end
end
