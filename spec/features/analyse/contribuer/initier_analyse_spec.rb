...
feature 'Initiation d’une nouvelle analyse' do
  ...
  before(:all) do
    ...
    if backup_base_biblio
      ...
      puts "Durée du backup de la base :biblio : #{laps}"
    end
  end
  before(:each) do
  ...
  after(:all) do
    ...
    laps  = (Time.now.to_f - start).round(5)
    puts "Durée du retrieve de la base :biblio : #{laps}"
  end
  ...
  scenario 'Un inscrit analyste peut initier une nouvelle analyse par le menu Filmodico' do
    ...
    expect(page).to have_tag('h3', text: hfilm[:titre])
    expect(page).to have_tag('div', text: /Données générales de l’analyse du film/)
