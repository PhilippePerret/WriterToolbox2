...
RSpec::Matchers::define :have_mails do |params|
  match do |qui|
    ...
    # Ne pas confontre "send_after" avec "sent_after"
    if @params.key?(:send_after) || @params.key?(:send_before)
      raise "`send` doit être remplacé par `sent` (avec un `t`) dans les propriétés de test `send_after` ou `send_before`."
    end
  ...
  failure_message do |qui|
    "Aucun mail n'a été trouvé avec les paramètres fournis. #{MailMatcher.message_added}#{MailMatcher.message_almost_founds}"
  end
